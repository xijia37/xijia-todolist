	...
	public static function timeAgoInWords($dateTime, $options = array()) {
		...
		if ($diff > abs($now - strtotime($end))) {
			...
			if ($years > 0) {
				...
				$relativeDate .= ($relativeDate ? ', ' : '') . $years . ' ' . ($years==1 ? Yii::t('comp', 'year') : Yii::t('comp', 'years'));
				$relativeDate .= $months > 0 ? ($relativeDate ? ', ' : '') . $months . ' ' . ($months==1 ? Yii::t('comp', 'month') : Yii::t('comp', 'months')) : '';
				$relativeDate .= $weeks > 0 ? ($relativeDate ? ', ' : '') . $weeks . ' ' . ($weeks==1 ? Yii::t('comp', 'week') : Yii::t('comp', 'weeks')) : '';
				$relativeDate .= $days > 0 ? ($relativeDate ? ', ' : '') . $days . ' ' . ($days==1 ? Yii::t('comp', 'day') : Yii::t('comp', 'days')) : '';
			} elseif (abs($months) > 0) {
				// months, weeks and days
				$relativeDate .= ($relativeDate ? ', ' : '') . $months . ' ' . ($months==1 ? Yii::t('comp', 'month') : Yii::t('comp', 'months'));
				$relativeDate .= $weeks > 0 ? ($relativeDate ? ', ' : '') . $weeks . ' ' . ($weeks==1 ? Yii::t('comp', 'week') : Yii::t('comp', 'weeks')) : '';
				$relativeDate .= $days > 0 ? ($relativeDate ? ', ' : '') . $days . ' ' . ($days==1 ? Yii::t('comp', 'day') : Yii::t('comp', 'days')) : '';
			} elseif (abs($weeks) > 0) {
				// weeks and days
				$relativeDate .= ($relativeDate ? ', ' : '') . $weeks . ' ' . ($weeks==1 ? Yii::t('comp', 'week') : Yii::t('comp', 'weeks'));
				$relativeDate .= $days > 0 ? ($relativeDate ? ', ' : '') . $days . ' ' . ($days==1 ? Yii::t('comp', 'day') : Yii::t('comp', 'days')) : '';
			} elseif (abs($days) > 0) {
				// days and hours
				$relativeDate .= ($relativeDate ? ', ' : '') . $days . ' ' . ($days==1 ? Yii::t('comp', 'day') : Yii::t('comp', 'days'));
				$relativeDate .= $hours > 0 ? ($relativeDate ? ', ' : '') . $hours . ' ' . ($hours==1 ? Yii::t('comp', 'hour') : Yii::t('comp', 'hours')) : '';
			} elseif (abs($hours) > 0) {
				// hours and minutes
				$relativeDate .= ($relativeDate ? ', ' : '') . $hours . ' ' . ($hours==1 ? Yii::t('comp', 'hour') : Yii::t('comp', 'hours'));
				$relativeDate .= $minutes > 0 ? ($relativeDate ? ', ' : '') . $minutes . ' ' . ($minutes==1 ? Yii::t('comp', 'minute') : Yii::t('comp', 'minutes')) : '';
			} elseif (abs($minutes) > 0) {
				// minutes only
				$relativeDate .= ($relativeDate ? ', ' : '') . $minutes . ' ' . ($minutes==1 ? Yii::t('comp', 'minute') : Yii::t('comp', 'minutes'));
			} else {
				// seconds only
				$relativeDate .= ($relativeDate ? ', ' : '') . $seconds . ' ' . ($seconds==1 ? Yii::t('comp', 'second') : Yii::t('comp', 'seconds'));
			}

			if (!$backwards) {
				$relativeDate = sprintf('%s ' . Yii::t('comp', 'ago'), $relativeDate);
			}
		}
		return $relativeDate;
	}
